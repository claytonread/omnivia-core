...
and the candidate catalogue now carries the Workflow application operations, but
``operation-traceability-v1.json`` still records only three provider-neutral
service adapters -- ``in_process``, ``ipc``, ``http`` -- and states every one of
their per-operation evidence states as pending rather than claiming a result.
...
holds and stays in step with the frozen twenty-six-operation catalogue.
...
* It copies no case from the accepted 85-case adapter-wire-conformance corpus.
...
#: failure names the operation rather than an index into a list of twenty-six.
...
def test_the_fixture_covers_exactly_the_frozen_twenty_six_operations_in_catalogue_order() -> None:
    assert len(OPERATION_CATALOGUE) == 26
    assert FIXTURE_NAMES == [entry.name for entry in OPERATION_CATALOGUE]
    assert len(FIXTURE_NAMES) == 26
    assert len(set(FIXTURE_NAMES)) == 26
...
def test_every_operation_reports_pending_candidate_for_every_service_adapter_class(
    ...
    every one of the 26 x 3 service-adapter evidence states must read exactly
...
# --------------------------------------------------------------------------
# The referenced 85-case adapter-wire-conformance corpus
# --------------------------------------------------------------------------
...
def test_the_fixture_references_the_accepted_corpus_by_name_and_format() -> None:
    ...
    assert reference["case_count"] == 85


def test_the_referenced_corpus_file_exists_and_holds_exactly_85_unique_cases() -> None:
    ...
    assert len(case_ids) == 85
    assert len(set(case_ids)) == 85
