...
holds and stays in step with the frozen twenty-three-operation catalogue.
...
* It copies no case from the accepted 78-case adapter-wire-conformance corpus.
...
def test_the_fixture_covers_exactly_the_frozen_operations_in_catalogue_order() -> None:
    ...
    assert len(set(FIXTURE_NAMES)) == 23
...
def test_the_fixture_references_the_accepted_corpus_by_name_and_format() -> None:
    ...
    assert reference["case_count"] == 78


def test_the_referenced_corpus_file_exists_and_holds_exactly_78_unique_cases() -> None:
    ...
    assert len(case_ids) == 78
    assert len(set(case_ids)) == 78
