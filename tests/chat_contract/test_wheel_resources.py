...
2. read the wheel's own file list and require *exactly* the 172 approved Chat
   Runtime Contract resources (13 schemas plus 160 fixture-tree files) under
...
def test_every_packaged_fixture_survives_packaging_byte_for_byte(
    ...
    assert installed_report["fixture_count"] == 159
...
def test_the_packaged_manifest_declares_the_governed_counts(
    ...
    assert installed_report["manifest_counts"] == {"total": 159, "valid": 76, "invalid": 83}
