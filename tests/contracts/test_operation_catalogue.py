...
def test_the_catalogue_holds_exactly_the_frozen_twenty_six_operations_in_order() -> None:
    ...
    assert len(set(FROZEN_NAMES)) == 26


def test_two_operations_are_installation_scoped_and_twenty_four_are_workspace_scoped() -> None:
    ...
    assert len(workspace) == 24
    assert len(installation) + len(workspace) == 26
...
def _emitted_typescript_catalogue() -> list[dict[str, Any]]:
    ...
    appended after the frozen twenty-six. The literal is delimited exactly rather
...
def test_the_readme_publishes_exactly_the_frozen_catalogue() -> None:
    ...
    workspace = _documented_operations("Twenty-four are workspace-scoped:")
    documented = installation + workspace

    assert sorted(documented) == FROZEN_NAMES
    assert len(documented) == len(set(documented)) == 26
    assert installation == [
...
def test_the_readme_operation_list_names_no_probe_and_no_job_resume(name: str) -> None:
    documented = _documented_operations("Two are installation-scoped:") + _documented_operations(
        "Twenty-four are workspace-scoped:"
    )
