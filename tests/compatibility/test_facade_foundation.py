"""The omnivia-memory facade foundation: five leaves plus three barrels.

Phase 1 ported portable leaves from ``services/omnivia-memory`` into
``src/omnivia_core`` as source-parity copies (see
``tests/canonical_migration``). This slice begins retiring that duplication
at the lowest level: ``_shared.validation``, ``lifecycle.models``,
``lifecycle.rules``, ``provenance.models``, and ``memory.models`` in
``omnivia_memory`` are no longer copies -- they are thin compatibility
facades whose supported symbols are the *exact* canonical objects
(``omnivia_memory.X.Symbol is omnivia_core.X.Symbol``), not structurally
equal lookalikes. The three barrels above them (``_shared``, ``lifecycle``,
``provenance``) already delegated to their sibling leaf and needed no source
change, but their re-exported objects are now canonical too as a result.
...
these five leaves via ``FACADE_CANONICAL_TO_LEGACY`` -- see
...
#: The five legacy leaves this slice converted into facades, and -- for each
#: supported symbol bound at that leaf's module scope -- the canonical
#: module that owns the exact object it must route to. Declared independently
#: of ``FACADE_CANONICAL_TO_LEGACY`` (rather than derived from it) so this
#: test also catches that manifest silently drifting, per
#: ``test_facade_canonical_to_legacy_manifest_matches_expected_five_pairs``.
#:
...
#: The three barrels, unchanged in this slice, and the exact ordered
#: ``__all__`` each must keep (architecture decision: preserve the existing
#: ordered literal list). Keyed by the shared package-relative suffix so
#: both trees (``omnivia_core.<suffix>`` / ``omnivia_memory.<suffix>``) can
...
#: suite now uses to exclude these five leaves from its source-parity gates.
...
@pytest.mark.parametrize("barrel", sorted(f"omnivia_memory.{b}" for b in BARREL_ALL_ORDER))
def test_barrel_ast_is_pure_facade(barrel: str) -> None:
    _assert_pure_facade_module(barrel, allow_all=True)


def test_facade_canonical_to_legacy_manifest_matches_expected_five_pairs() -> None:
    """``FACADE_CANONICAL_TO_LEGACY`` (imported from the migration-test
    manifest) must be exactly these five pairs -- neither manifest may drift
