...
# ruff: noqa: I001
# I001 would resort this import block. This module is a 1:1 port of
# `omnivia_memory.knowledge.validation` and tests/canonical_migration/test_parity.py
# gates it on normalized AST parity: `ast.dump` of this module must equal `ast.dump`
# of the legacy module with only `omnivia_memory` -> `omnivia_core` rewritten, so the
# import statements' exact name order must survive verbatim. Comments are the only
# thing that gate cannot see, so the suppression is the fix; resorting the import
# has to land in the legacy source first.
