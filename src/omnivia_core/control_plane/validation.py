...
from omnivia_core._shared.validation import scan_sensitive_fields
...
from omnivia_core.knowledge.validation import check_contract_version_compatibility
...
def validate_control_plane_manifest(
    ...
    for import_record in manifest_obj.import_records:
        ...
        for capability_id in import_record.candidate_capability_ids:
            ...
            capability = _find_by_id(manifest_obj.capabilities, capability_id)  # type: ignore[assignment]
            if (
    ...
    for secret_metadata in manifest_obj.secret_metadata:
        ...
        if secret_metadata.storage_scope == SecretStorageScope.CLIENT_OWNED:  # noqa: SIM102
...
class _PolicyExpressionParser:
    ...
    def parse(self) -> PolicyAttributeExpression:
        ...
        if self._index != len(self._tokens):
            kind, value = self._tokens[self._index]  # noqa: RUF059
            raise ControlPlaneValidationError(
