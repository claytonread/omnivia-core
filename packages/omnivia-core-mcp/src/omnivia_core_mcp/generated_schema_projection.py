...
SCHEMAS: Final[dict[str, dict[str, Any]]] = {
    ...
    "https://contracts.omnivia.dev/application/v1/context-pack.schema.json#/$defs/ContextPackBuildResult": {
        ...
        "properties": {
            ...
            "evidence": {
                "type": "array",
                "description": "Selected L0 evidence artifacts, ordered deterministically by identifier then content checksum. Every artifact is held to the complete resolution-time closure: `canonical_resolution_time` is an inclusive upper bound on every act and observation the artifact carries, not only on the instants this partition's own rule reads -- `temporal.event_at`, `observed_at`, `ingested_at` and `recorded_at`, `source.retrieved_at`, every `provenance_history[].occurred_at`, and every `provenance_history[].evidence[].source.retrieved_at`. Equality passes and only a strictly later instant is refused, and the refusal is a refusal rather than a repair: provenance is append-only, so an out-of-range instant is never dropped or truncated to make the artifact selectable. Its validity must contain the resolution instant inclusively -- `temporal.valid_from` no later than it and `temporal.valid_until` no earlier than it, both boundaries accepted at equality. `temporal.superseded_at` must be absent or strictly after the resolution instant: supersession is exclusive where the closure is inclusive, because an artifact replaced *at* the instant a pack resolved was already not the live one, so equality is rejected here rather than accepted.",
                "items": {
            ...
            "context_models": {
                "type": "array",
                "description": "Selected current, canonical L3 context-model governed records, ordered deterministically by record identifier then version. May be empty. Held to exactly the same current rules `records` states, at L3 rather than L2: the same complete resolution-time closure over the same nested provenance paths, inclusive at equality and refused rather than repaired past it; the same inclusive validity containment of the resolution instant at both boundaries; and the same requirement to be current and unsuperseded at that instant, with `provenance.temporal.superseded_at` absent outright, irrespective of timestamp -- a value strictly after the resolution instant is refused exactly as one at or before it is. Only the governance layer differs -- `layer` exactly `l3` -- and a context model is otherwise no more selectable than an L2 record would be under the same temporal facts.",
                "items": {
                    "$ref": "#/$defs/memory__GovernedRecord",
                },
                "maxItems": 500,
