...
* **C-15** the handler's own `len(rows) != 1` refusal, reached on a workspace that is
  genuinely one migration short of 0037, so the branch is exercised without the 0037
...
#: The last migration before 0037. A workspace stopped here is what C-15 needs: the
#: unique index does not exist yet, so the state the handler refuses is reachable.
PRE_IDENTITY_VERSION: Final = 36
...
def test_the_same_source_id_in_two_workspaces_is_two_independent_artifacts(
    ...
    `test_0037_keeps_genuinely_different_identities_apart` shows the database keeps two
    ...
    with workspaces(tmp_path, "ws-capture-0001", "ws-capture-0002") as (first, second):
        ...
        for workspace, result in zip((first, second), results, strict=True):
            ...
            # whole direct-submission identity tuple 0037 makes unique.
            assert rows(
                owned,
...
def test_a_source_identity_naming_two_artifacts_fails_closed_and_writes_nothing(
    ...
    """C-15's handler half, on a workspace 0037 has genuinely not reached.
    ...
    picking one would publish a guess. 0037 makes that state unreachable once applied,
    ...
    forge_a_second_source` uses to prove 0037 refuses it at the head. Nothing here
    ...
    try:
        ...
        # The row 0037 would have refused. Legal here, and exactly what the handler
...
def test_two_concurrent_identical_captures_produce_one_durable_effect(
    ...
    nothing about the race visible to either caller. `test_0037_refuses_the_repeat_in_a_
