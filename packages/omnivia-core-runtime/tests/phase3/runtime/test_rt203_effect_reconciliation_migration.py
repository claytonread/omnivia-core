...
def seed_unknown_source(holder: m1.Owned, *, dispatched: bool = False) -> None:
    """An intent settled `unknown`, either never dispatched or dispatched without a receipt.

    NOT_APPLIED reconciliation is only lawful for the never-dispatched half, and a receipt
    only exists for the dispatched half, so which crash window the source came from decides
    which reconciliation it can reach.
    """
    if dispatched:
