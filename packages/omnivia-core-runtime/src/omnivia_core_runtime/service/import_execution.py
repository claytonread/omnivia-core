...
2. The artifact and its first provenance event commit together in one fenced
   ...
   what makes a retry safe, and it is not the only thing making it safe: 0037's unique
...
class ImportJobExecutor:
    ...
    def _import(self, job_id: str) -> Mapping[str, Any]:
        ...
        rule 0037 makes the database's rather than a caller's. Nothing here can fail
