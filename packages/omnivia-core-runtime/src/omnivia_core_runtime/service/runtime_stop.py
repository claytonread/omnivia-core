    ...
    *different* terminal status is refused: 0018 makes every terminal status a sink, so
    ...
    with runtime_writer(
        ...
        if status == RUN_STATUS_CANCELLED:
            writer.record_run_stop_outcome(
                stop_request_id=run.stop.run_stop_id,
                outcome="accepted",
                completed_at_us=settled_at_us,
                runtime_event_sequence=read_run_sequence(
                    connection, workspace_id=workspace_id, run_id=run_id
                ),
                reason=reason,
                audit_ref=run.stop.audit_reference,
            )
        return status
