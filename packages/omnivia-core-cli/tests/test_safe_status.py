...
def _selection(home: Path, workspace_id: str = WORKSPACE_ID) -> tuple[Path, str]:
    installation_state = home / "installation-state"
    installation_state.mkdir(parents=True, exist_ok=True)
    return installation_state, workspace_id
...
def test_a_resolved_target_is_local_locally_managed_and_named_for_neither(
    ...
    target = resolve_local_target(*_selection(tmp_path / "home"))
...
def test_the_two_references_are_stable_opaque_and_distinct(tmp_path: Path) -> None:
    ...
    installation_state, workspace_id = _selection(home)

    first = resolve_local_target(installation_state, workspace_id)
    second = resolve_local_target(installation_state, workspace_id)
    ...
    other = resolve_local_target(*_selection(tmp_path / "other-home"))
...
def test_two_aliases_of_one_home_resolve_to_one_target(tmp_path: Path) -> None:
    ...
    through_link = resolve_local_target(alias / "installation-state", workspace_id)
    direct = resolve_local_target(installation_state, workspace_id)

    assert direct is not None and through_link is not None
    assert through_link == direct


@pytest.mark.parametrize("workspace_id", ["not a workspace id", "", "x" * 129])
def test_a_malformed_explicit_workspace_resolves_no_target(
    tmp_path: Path, workspace_id: str
) -> None:
    assert resolve_local_target(tmp_path / "installation-state", workspace_id) is None


def _target(tmp_path: Path) -> Any:
    resolved = resolve_local_target(*_selection(tmp_path / "home"))
...
def test_no_published_status_carries_a_forbidden_key_or_the_home_it_came_from(
    ...
    home = tmp_path / "sentinel-home"
    installation_state, workspace_id = _selection(home, workspace_id=WORKSPACE_ID)
    target = resolve_local_target(installation_state, workspace_id)
    assert target is not None
    ...
    for status in _every_status(target):
        ...
        _assert_publishable(
            document,
            sentinels=("sentinel-home", str(home), str(installation_state)),
        )
